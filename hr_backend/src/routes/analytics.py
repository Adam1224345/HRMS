...
def get_admin_dashboard_data():
    try:
        ...
        # Leave by Type (SQLite-safe, capitalize in Python)
        ...
        # Monthly Trend (SQLite-safe with strftime)
...
def get_employee_dashboard_data():
    try:
        ...
        monthly_raw = db.session.query(
            func.strftime('%Y-%m', Leave.start_date),
            func.count(Leave.id)
        ).filter(
            Leave.user_id == user.id,
            Leave.start_date >= six_months_ago
        ).group_by(func.strftime('%Y-%m', Leave.start_date))\
         .order_by(func.strftime('%Y-%m', Leave.start_date)).all()
