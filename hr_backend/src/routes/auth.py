...
def login():
    try:
        ...
        # SAFELY extract jti from refresh_token
        try:
            decoded = decode_token(refresh_token)
            refresh_jti = decoded.get("jti")
            exp = decoded.get("exp")
            expires_at = (
                datetime.utcfromtimestamp(exp) if exp else datetime.utcnow() + timedelta(days=7)
            )

            RefreshToken.add_token(user.id, refresh_jti, expires_at)

        except Exception as e:
            # fallback: store refresh entry with no jti
            RefreshToken.add_token(
                user.id, None, datetime.utcnow() + timedelta(days=7)
            )
...
def refresh():
    try:
        ...
        try:
            ...
            RefreshToken.add_token(
                int(user_id), None, datetime.utcnow() + timedelta(days=7)
            )
...
def check_if_token_revoked(jwt_header, jwt_payload):
    """Check if JWT access token is blacklisted or revoked in DB."""
