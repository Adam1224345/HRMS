...
# === SWAGGER DOCUMENTATION (Updated - No inline definitions) ===
get_leaves_docs = {
    "tags": ["Leave"],
    ...
    "parameters": [
        ...
        {"name": "status", "in": "query", "type": "string", "enum": ["Pending", "Approved", "Rejected"], "description": "Filter by status"}
    ],
    "responses": {
        "200": {
...
def get_leave(leave_id):
    try:
        ...
        return jsonify(leave.to_dict()), 200  # Direct object
...
def create_leave():
    try:
        ...
        return jsonify(leave.to_dict()), 201  # Direct object
...
def update_leave(leave_id):
    try:
        ...
        # Admin/HR can update status/remarks
        if is_admin_hr:
        ...
        # Owner can edit details if pending
        if is_owner_pending:
...
def approve_leave(leave_id):
    return _change_status(leave_id, 'Approved', approve_leave_docs)
...
def reject_leave(leave_id):
    return _change_status(leave_id, 'Rejected', reject_leave_docs)


# === HELPER FUNCTION ===
def _change_status(leave_id, status, docs):
    try:
        current_user_id = 1 if current_app.config.get("DEBUG", False) else get_jwt_identity()
        ...
        db.session.rollback()
        return jsonify({'error': str(e)}), 500@debug_skip_auth
def get_leaves():
    try:
        if current_app.config.get("DEBUG", False):
            leaves = Leave.query.all()
...
def reject_leave(leave_id):
    try:
        ...
        db.session.rollback()
        return jsonify({'error': str(e)}), 500    "summary": "Get all leave requests (paginated)",
    "description": "Admins/HR see all leaves. Employees see only their own.",
    "parameters": [
        {"name": "page", "in": "query", "type": "integer", "default": 1, "description": "Page number"},
        {"name": "per_page", "in": "query", "type": "integer", "default": 10, "description": "Items per page"},
