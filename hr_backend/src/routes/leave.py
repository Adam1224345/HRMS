...
# ===================================================================
# The library calculates estimated dates. For 2025, we use the official circular for 100% accuracy.
...
# Define the Leave schema explicitly so $ref pointers work
leave_definition = {
    "type": "object",
    "properties": {
        "id": {"type": "integer"},
...
# --- NEW: Smart Leave Analysis Endpoint ---
@leave_bp.route('/leaves/analyze-dates', methods=['POST'])
@jwt_required()
def analyze_leave_dates():
    """
    Smart Suggestions: Works for ANY future year using the 'holidays' library.
    Checks for overlaps, holidays, and suggests bridge days.
    """
    ...
    # 1. GENERATE HOLIDAYS FOR THE REQUESTED YEAR
    # This automatically supports future years (e.g., 2026, 2027)
    pk_holidays = holidays.PK(years=start_date.year)
    
    # OVERRIDE: If the year is 2025, use our precise official list
    ...
    # Logic: If 1 day before/after is a bridge to a holiday/weekend, suggest taking it.
...
def delete_leave(leave_id):
    ...
    # Debug prints removed for production readiness, re-enable if needed
    try:
        current_user_id = int(current_user_id)
    except ValueError:
