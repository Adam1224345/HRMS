...
const LeaveManagement = () => {
  ...
  const openFile = async (downloadUrl, fileName) => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute('download', fileName || t('document'));
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <Badge variant={STATUS_MAP[status]?.variant || 'secondary'} className="flex items-center gap-1">
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-bold text-gray-900">{t('leave_management_title')}</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-gray-600">{t('stat_total')}</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold">{stats.total}</div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-yellow-600">{t('stat_pending')}</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-yellow-600">{stats.pending}</div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-green-600">{t('stat_approved')}</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-green-600">{stats.approved}</div></CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-red-600">{t('stat_rejected')}</CardTitle></CardHeader><CardContent><div className="text-2xl font-bold text-red-600">{stats.rejected}</div></CardContent></Card>
      </div>

      <div className="flex justify-between items-center">
        <Button disabled={page === 1} onClick={() => setPage((prev) => prev - 1)} variant="outline">{t('previous')}</Button>
        <span>{t('page_of_total', { page, total: totalPages })}</span>
        <Button disabled={page === totalPages} onClick={() => setPage((prev) => prev + 1)} variant="outline">{t('next')}</Button>
      </div>
        ...
        <DialogContent className="max-w-md max-h-[90vh] overflow-y-auto">
          ...
          {selectedLeave && (
            <div className="space-y-4">
              ...
              <div><Label className="text-gray-600">{t('attached_documents')}</Label><DocumentList documents={viewDocuments} /></div>
            ...
            <DialogDescription>
              {selectedLeave && (
                <div className="mt-4 space-y-2 text-sm">
                  ...
                  <div className="pt-2"><strong>{t('attached_documents_colon')}:</strong><DocumentList documents={viewDocuments} /></div>
                </div>
              )}
            </DialogDescription>
          </DialogHeader>
